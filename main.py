...
class SyncService:
    ...
    def onMessage(self, data: dict):
        ...
        if data.get('type') =='probe':
            devId = data.get('dev_id')
            index = data.get('from')
            deviceData = next((data for data in self.store if data.get('devId')== devId),None)
            if deviceData is None:
                return 
            updateData = {"type":'update','from':'index','data':deviceData['data'][index:]}
            return updateData
        else:    
